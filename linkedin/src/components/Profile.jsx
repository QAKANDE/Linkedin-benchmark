...
class Profile extends Component {
  ...
  handleUpload = async () => {
    ...
    let response = await fetch(
      "https://striveschool.herokuapp.com/api/profile/user23/picture",
      {
        method: "POST",
        body: photo,
        headers: new Headers({
  ...
  componentDidMount = async () => {
    ...
    let response = await fetch(
      "http://localhost:3008/profile/linkedin/" + email,
      {
        method: "GET",
        headers: new Headers({
    ...
    console.log("benchmark", this.state.details);
          ...
          return (
            <div key={index}>
              ...
              <div className="d-flex flex-row" style={{ height: "75px" }}>
                <div id="imageWrapper">
                  <div id="profile-image-wrapper">
                    <img id="profilePic" src={detail.image} alt="Profile pic" />
                    <div id="bottom-right">
              ...
              <div className="container d-flex justify-content-between">
                <h3 className="ml-3">
                  {this.state.details.name} {this.state.details.surname}
                </h3>
                <span className="ml-3">{this.state.details.email}</span>
              </div>
              <div className="container d-flex justify-content-between">
                <h5 style={{ fontWeight: "normal" }} className="ml-3">
                  {this.state.details.area}
                </h5>
                <h5 style={{ fontWeight: "normal" }} className="ml-3">
